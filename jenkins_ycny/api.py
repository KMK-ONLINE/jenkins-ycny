...
class Report(JsonAble):
    ...
    def __repr__(self):
        return "Report for " + repr(self._build) + \
            " with %d fails, %d skips, %d passes for " % (
            self.failCount,
...
class Case(object):
    ...
    def __repr__(self):
        return "Suite %s | Case %s | %s with status %s" % (
            self._suite.name, self.className, self.name, self.status)
